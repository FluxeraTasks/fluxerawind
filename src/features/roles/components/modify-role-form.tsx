...
import { modifyRoleSchema } from "../schema";
...
import { useModifyRole } from "../api/use-modify-role";
...
export const ModifyRoleForm = ({ onCancel, role }: ModifyRoleFormProps) => {
  const { mutate, isPending } = useModifyRole();
  ...
  const form = useForm<z.infer<typeof modifyRoleSchema>>({
    ...
    defaultValues: {
      ...
      getArtifact: role.getArtifact,
      getProject: role.getProject,
      getTask: role.getTask,
      postArtifact: role.postArtifact,
      postProject: role.postProject,
      postTask: role.postTask,
      putArtifact: role.putArtifact,
      putProject: role.putProject,
      putTask: role.putTask,
      delArtifact: role.delArtifact,
      delProject: role.delProject,
      delTask: role.delTask,
    },
  });

  const onSubmit = (values: z.infer<typeof modifyRoleSchema>) => {
